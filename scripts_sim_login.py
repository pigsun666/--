...
def dump_result(result: CapturedLoginResult, dump_request: bool) -> None:
    payload: dict[str, Any] = {
        "request_url": result.request_url,
        "request_method": result.request_method,
        "response_status": result.response_status,
        "response_headers": result.response_headers,
        "response_body": _safe_json_loads(result.response_body) or result.response_body,
        "token": result.token or "",
    }
    if dump_request:
        payload["request_headers"] = result.request_headers
        payload["request_post_data"] = _safe_json_loads(result.request_post_data or "") or result.request_post_data
...
async def main() -> int:
    ...
    parser.add_argument("--dump-request", action=argparse.BooleanOptionalAction, default=True, help="是否附带输出请求头和请求体")
    ...
    dump_result(result, cfg.dump_request)
    return 0
